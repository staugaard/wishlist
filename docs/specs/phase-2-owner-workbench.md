...
| Email sender | **Cloudflare Email Service** via the `send_email` Workers binding (decided 2026-08-14, superseding the Resend plan): no API key, no external vendor; sender `hinted@season4.app` (the user's existing domain, DNS moved to Cloudflare). Family members are verified destination addresses — sends to them are free on any plan. Dev builds log the code instead of sending | Fewest moving parts — the deciding criterion everywhere else — and it removes the only planned third-party service |
