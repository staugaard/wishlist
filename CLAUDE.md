...
- Adding a family member (no invite UI, by design): `pnpm exec wrangler d1 execute wishlist-db --remote --command "INSERT INTO users (email, name, created_at) VALUES ('them@example.com', 'Their Name', unixepoch())"` — plus `pnpm exec wrangler email routing addresses create them@example.com` (they click one verification link; sends to verified destinations are free).
- The magic paste (`src/lib/{metadata,images,enrich}.ts`): pasting a URL creates the row instantly, then `waitUntil` enrichment fills title/price/image with **fill-if-untouched conditional writes** — never unconditional updates. All fetchers take an injectable `fetcher` param (the test seam; vitest 4 has no fetchMock). Product images are copied into the `IMAGES` R2 bucket and served at `/img/*` (key format strictly validated); never hotlink merchant images.
